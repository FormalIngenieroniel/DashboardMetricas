...
valid_paises_model = set(dynamic_categories.get('pais', []))
  ...
  if sectores_validos_en_map:
        ...
        if not filtered_mapping and valid_paises_model: 
...
with col_form:
    ...
    if not paises_options: 
        paises_options = dynamic_categories.get('pais', ['Colombia']) 
...
(studios)", [0, 1], index=default_property_base['studios'], format_func=lambda x: "Sí" if x == 1 else "No")
    ...
    sector_input = c8.selectbox(
        ...
        index=idx_sector,
        key=f"sector_selector_{pais_input}" 
...
- 1) * 0.3
                        ...
                        base_coef_bed = (bedroom_diff * 0.3) if bedroom_diff == 1 else 0.3 + (bedroom_diff-1)*0.15 
                        property_factor_ajuste = 1.0 + base_coef_bath + base_coef_bed
                        if bathroom_diff ==1 
and bedroom_diff ==1: property_factor_ajuste=2.5 
...
base modelo: COP {prediccion:,.2f}"]
                    if pais_factor_ajuste != 1.0 or (pais_to_use != base_pais_comparison and pais_to_use not in specific_country_factors) : 
         ...
         if property_factor_ajuste != 1.0:
          ...
          if prediccion_final != prediccion : 
...
if 'property_base_simulacion' not in st.session_state or 'precio_base_simulacion' not in st.session_state:
    ...
    property_base_actual_dict_sim = st.session_state.property_base_simulacion 
    price_base_actual_sim = st.session_state.precio_base_simulacion 
...
(% de Ingresos)", 0.0, 100.0, 20.0, 1.0) / 100.0
    ...
    if st.button("🏦 Calcular Rentabilidad de Escenarios", key="calculate_roi", use_container_width=True):
        if modelo:
            ...
            base_features_for_scenario = st.session_state.get('last_input_features', property_base_actual_dict_sim) 
     ...
     if pais_factor_for_scenarios != 0 and last_prop_factor_interactive != 0:
                    ...
                    raw_pred_base_estimada = price_base_final_scenario 
